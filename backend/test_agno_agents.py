...
from app.agents.models import get_extraction_model, get_structured_model, get_reasoning_model
...
def test_accuracy_with_structured_output():
    ...
    try:
        ...
        if result and result.avg_score >= 8:
            ...
            print(f"❌ ACCURACY TEST NEEDS WORK: {result.avg_score if result else 'N/A'}/10")
...
def test_performance_benchmarks():
    ...
    try:
        ...
        print(f"✅ Structured agents: {structured_result.avg_run_time*1000:.1f}ms average")
...
def test_reliability_and_tools():
    ...
    def test_python_tools_security():
        ...
        try:
            with tempfile.TemporaryDirectory() as secure_dir:
                ...
                assert tools.base_dir == secure_dir, "base_dir must match specified directory"
                assert Path(secure_dir).exists(), "Working directory must be created"
                
                # Verify tool methods
                assert hasattr(tools, 'run_python_code'), "Must have code execution capability"
    ...
    def test_model_consistency():
        ...
        try:
            ...
            assert all(m.id == first_model_id for m in models), "Models should be consistent"
...
def test_end_to_end_user_workflow():
    ...
    def complete_extraction_workflow():
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config = prompt_workflow.run("Extract vendor, invoice number, amount from invoices")
            ...
            responses = list(data_workflow.run(
                "Extract provider name, contract number, total value, and service items",
                [{"name": "contract.txt", "path": str(doc_file), "type": "text/plain", "size": len(contract_content)}]
            ))
